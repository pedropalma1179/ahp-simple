...
export default function ResultadosPage() {
  ...
  const runAiReview = async () => {
    ...
    try {
      // Preparar resumo demográfico para o revisor
      const demographicSummary = respondentsDemographics.length > 0 ? {
        total: respondentsDemographics.length,
        gender: {
          masculino: respondentsDemographics.filter(d => d.genero === 'masculino').length,
          feminino: respondentsDemographics.filter(d => d.genero === 'feminino').length
        },
        age: {
          menos_30: respondentsDemographics.filter(d => d.idade === 'menos_30').length,
          '31_40': respondentsDemographics.filter(d => d.idade === '31_40').length,
          '41_50': respondentsDemographics.filter(d => d.idade === '41_50').length,
          mais_50: respondentsDemographics.filter(d => d.idade === 'mais_50').length
        },
        education: {
          graduacao: respondentsDemographics.filter(d => d.formacao === 'superior').length,
          especializacao: respondentsDemographics.filter(d => d.formacao === 'especializacao').length,
          mestrado: respondentsDemographics.filter(d => d.formacao === 'mestrado').length,
          doutorado: respondentsDemographics.filter(d => d.formacao === 'doutorado').length
        },
        experience: {
          menos_10: respondentsDemographics.filter(d => d.tempoTrabalho === 'menos_10').length,
          '11_20': respondentsDemographics.filter(d => d.tempoTrabalho === '11_20').length,
          '21_30': respondentsDemographics.filter(d => d.tempoTrabalho === '21_30').length,
          mais_30: respondentsDemographics.filter(d => d.tempoTrabalho === 'mais_30').length
        },
        roles: {
          c_level: respondentsDemographics.filter(d => d.funcao === 'c_level').length,
          diretor: respondentsDemographics.filter(d => d.funcao === 'diretor').length,
          gerente: respondentsDemographics.filter(d => d.funcao === 'gerente').length,
          supervisor: respondentsDemographics.filter(d => d.funcao === 'supervisor').length,
          analista: respondentsDemographics.filter(d => d.funcao === 'analista').length
        },
        areas: {
          producao: respondentsDemographics.filter(d => d.areaAtuacao === 'producao').length,
          eng_processos: respondentsDemographics.filter(d => d.areaAtuacao === 'eng_processos').length,
          qualidade: respondentsDemographics.filter(d => d.areaAtuacao === 'qualidade').length,
          manutencao: respondentsDemographics.filter(d => d.areaAtuacao === 'manutencao').length,
          logistica: respondentsDemographics.filter(d => d.areaAtuacao === 'logistica').length,
          ti: respondentsDemographics.filter(d => d.areaAtuacao === 'ti').length,
          financas: respondentsDemographics.filter(d => d.areaAtuacao === 'financas').length
        }
      } : null;
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="space-y-6">
                {calculation.finalScores.map((alt, idx) => {
                  // Tentar usar valores diretos ou calcular a partir de altScores
                  let b = alt.B || 0;
                  let o = alt.O || 0;
                  let c = alt.C || 0;
                  let r = alt.R || 0;
                  ...
                  const maxVal = Math.max(b + o, c + r, 0.01);
                  
                  return (
                    <div key={idx} className="relative">
                      <div className="text-sm font-medium text-gray-700 mb-2">{alt.name}</div>
                      <div className="flex items-center h-10">
                        {/* Lado negativo (C+R) */}
                        <div className="flex-1 flex justify-end">
                          <div 
                            className="h-8 bg-red-400 rounded-l flex items-center justify-start pl-2"
                            style={{ width: `${((c + r) / maxVal) * 50}%`, minWidth: c + r > 0.001 ? '20px' : '0' }}
                          >
                            {c + r > 0.05 && (
                              <span className="text-white text-xs">C+R: {((c + r) * 100).toFixed(1)}%</span>
                            )}
                          </div>
                        </div>
                        {/* Centro */}
                        ...
                        <div className="flex-1">
                          <div 
                            ...
                            style={{ width: `${((b + o) / maxVal) * 50}%`, minWidth: b + o > 0.001 ? '20px' : '0' }}
                          >
                            {b + o > 0.05 && (
                              <span className="text-white text-xs">B+O: {((b + o) * 100).toFixed(1)}%</span>
                            )}
                          </div>
                        </div>
                      </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="flex justify-center">
                <svg viewBox="0 0 400 400" className="w-full max-w-md">
                  ...
                  {/* Labels dos eixos */}
                  <text x="200" y="30" textAnchor="middle" className="text-sm fill-green-600 font-semibold">Benefícios</text>
                  <text x="370" y="205" textAnchor="start" className="text-sm fill-blue-600 font-semibold">Oportunidades</text>
                  <text x="200" y="380" textAnchor="middle" className="text-sm fill-orange-600 font-semibold">Custos</text>
                  <text x="30" y="205" textAnchor="end" className="text-sm fill-red-600 font-semibold">Riscos</text>
                  
                  {/* Polígonos das alternativas */}
                  {calculation.finalScores.map((alt, altIdx) => {
                    ...
                    const norm = (v: number) => Math.min(v / maxVal, 1);
                    
                    const values = [norm(b), norm(o), norm(c), norm(r)];
                    const angles = [0, 90, 180, 270];
                    
                    const points = angles.map((angle, i) => {
                      const rad = (angle - 90) * Math.PI / 180;
                      const radius = values[i] * 150;
                      ...
                      <g key={altIdx}>
                        ...
                        {angles.map((angle, i) => {
                          ...
                          return (
                            <circle
                              key={i}
                              cx={200 + radius * Math.cos(rad)}
                              cy={200 + radius * Math.sin(rad)}
                              r="4"
                              fill={color}
                            />
                          );
                ...
                <div className="space-y-6">
                  ...
                  <div className={`p-6 rounded-xl border-2 ${
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="text-center">
                        <p className={`text-4xl font-bold ${
                          aiReview.nota_geral >= 90 ? 'text-green-600' :
                          aiReview.nota_geral >= 75 ? 'text-blue-600' :
                          aiReview.nota_geral >= 60 ? 'text-yellow-600' : 'text-red-600'
                        }`}>{aiReview.nota_geral}/100</p>
                        <p className="text-sm text-gray-500">Nota Geral</p>
                      </div>
                    </div>
                    {aiReview.resumo_executivo && (
                      <p className="text-gray-700 bg-white/50 p-3 rounded-lg">{aiReview.resumo_executivo}</p>
                    )}
                  </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    {aiReview.validacoes_matematicas && (
                      <div className="bg-white rounded-xl p-4 border shadow-sm">
                        <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
                          🧮 Validações Matemáticas
                        </h4>
                        <div className="space-y-2 text-sm">
                          {Object.entries(aiReview.validacoes_matematicas).map(([key, val]: [string, any]) => (
                            <div key={key} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                              <span className="text-gray-600">{key.replace(/_/g, ' ')}</span>
                              <span className={`font-medium ${
                                val.status === 'OK' ? 'text-green-600' :
                                val.status === 'ALERTA' ? 'text-yellow-600' : 'text-red-600'
                              }`}>
                                {val.status === 'OK' ? '✓' : val.status === 'ALERTA' ? '⚠' : '✗'} {val.valor || val.status}
                              </span>
                            </div>
                          ))}
                        </div>
                    ...
                    {aiReview.validacoes_metodologicas && (
                      <div className="bg-white rounded-xl p-4 border shadow-sm">
                        <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
                          🔬 Validações Metodológicas
                        </h4>
                        <div className="space-y-2 text-sm">
                          {Object.entries(aiReview.validacoes_metodologicas).map(([key, val]: [string, any]) => (
                            <div key={key} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                              <span className="text-gray-600">{key.replace(/_/g, ' ')}</span>
                              <span className={`font-medium ${
                                val.status === 'OK' ? 'text-green-600' : 'text-yellow-600'
                              }`}>
                                {val.status === 'OK' ? '✓' : '⚠'} {val.status}
                              </span>
                            </div>
                          ))}
                        </div>
                      ...
                      <div className="bg-white rounded-xl p-4 border shadow-sm">
                        ...
                        <div className="space-y-2 text-sm">
                          <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                            ...
                            <span className={`font-medium ${
                              aiReview.analise_robustez.concordancia_metodos?.status === 'FORTE' ? 'text-green-600' :
                              aiReview.analise_robustez.concordancia_metodos?.status === 'MODERADA' ? 'text-yellow-600' : 'text-red-600'
                            }`}>
                              {aiReview.analise_robustez.concordancia_metodos?.valor}
                            </span>
                          </div>
                          <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                            <span className="text-gray-600">Estabilidade do ranking</span>
                            <span className={`font-medium ${
                              aiReview.analise_robustez.estabilidade_ranking?.status === 'ESTÁVEL' ? 'text-green-600' :
                              aiReview.analise_robustez.estabilidade_ranking?.status === 'SENSÍVEL' ? 'text-yellow-600' : 'text-red-600'
                            }`}>
                              {aiReview.analise_robustez.estabilidade_ranking?.status}
                            </span>
                          </div>
                          <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                            <span className="text-gray-600">Diferença 1º-2º lugar</span>
                            <span className={`font-medium ${
                              aiReview.analise_robustez.diferenca_1o_2o?.status === 'OK' ? 'text-green-600' :
                              aiReview.analise_robustez.diferenca_1o_2o?.status === 'EMPATE_TECNICO' ? 'text-red-600' : 'text-yellow-600'
                            }`}>
                              {aiReview.analise_robustez.diferenca_1o_2o?.valor}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    {aiReview.analise_amostra && (
                      <div className="bg-white rounded-xl p-4 border shadow-sm">
                        ...
                        <div className="text-center py-3">
                          <p className="text-3xl font-bold text-gray-800">n = {aiReview.analise_amostra.tamanho}</p>
                  ...
                  {aiReview.recomendacoes && aiReview.recomendacoes.length > 0 && (
                    <div className="bg-amber-50 rounded-xl p-4 border border-amber-200">
                      ...
                      <ol className="space-y-2">
                        {aiReview.recomendacoes.map((rec: string, idx: number) => (
                          <li key={idx} className="flex items-start gap-2 text-sm text-amber-700">
                  ...
                  {aiReview.adequacao_publicacao && (
                    <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-200">
                      <h4 className="font-semibold text-indigo-800 mb-3">📚 Adequação para Publicação</h4>
                      <div className="flex items-center gap-4">
                        ...
                        <p className="text-sm text-indigo-700">{aiReview.adequacao_publicacao.justificativa}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {aiReview.observacoes_revisor && (
                    <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button
                          onClick={() => {
                            navigator.clipboard.writeText(aiReview.observacoes_revisor);
                            alert('Observações copiadas!');
                          }}
