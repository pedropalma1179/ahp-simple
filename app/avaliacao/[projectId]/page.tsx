...
import { BOCR_CRITERIA, SUBCRITERIA, MAGNITUDE_NAMES, MAGNITUDE_DESCRIPTIONS, MAGNITUDE_QUESTIONS, MAGNITUDE_HELP_TEXT, generateAllComparisons, Project, Alternative, Respondent } from '@/lib/data';
...
export default function AvaliacaoProjectPage() {
  ...
  const getComparisonTitle = () => {
    ...
    if (current.type === 'magnitude') return '� Comparação de Impacto Total';
  ...
  const getQuestionText = () => {
    ...
    if (current.type === 'bocr') {
      return 'Considerando a decisão de investimento em tecnologias da Indústria 4.0, qual critério estratégico tem maior importância?';
    } 
  
    if (current.type === 'magnitude') {
      const key = `${current.itemA}_${current.itemB}` as keyof typeof MAGNITUDE_QUESTIONS;
      return MAGNITUDE_QUESTIONS[key] || 'Comparando as magnitudes totais:';
    }
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden">
          <div className="p-4 sm:p-6 bg-gradient-to-r from-indigo-600 to-purple-600">
            ...
            {/* 🆕 MAGNITUDE: Texto de ajuda */}
            {current && current.type === 'magnitude' && (
              <p className="text-white/70 text-sm mt-3 bg-white/10 rounded-lg p-3">
                {MAGNITUDE_HELP_TEXT[`${current.itemA}_${current.itemB}` as keyof typeof MAGNITUDE_HELP_TEXT]}
              </p>
            )}
          </div>
