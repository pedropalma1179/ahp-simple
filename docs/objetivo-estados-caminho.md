...
Documento âncora. Revisado em 11/09/2026 sobre o commit `a8aa904`.
...
| A.17 | **Trocar a fonte da citação que vai ao modelo: `verbatim_quote` → `evidence.quote`.** Uma linha, a **101** do `app/api/ai-reviewer/knowledge.ts`: `rule: claim.verbatim_quote || claim.claim`. É o campo defeituoso que chega ao modelo, e a linha 286 do mesmo arquivo **já faz o certo** para os limiares, usando `t.evidence.quote`. ⚠ **Resolve os sete casos de uma vez** e não depende de corrigir as treze divergências no RAG (A.16): o teste **encontra** as divergências, a troca de fonte **deixa de propagá-las**. Ordem: esta primeiro, porque é barata e imediata. **Verificado:** todas as 123 claims com `verbatim_quote` têm `evidence.quote`, e nenhuma tem só um dos dois, então o fallback é precaução. Evidência em `docs/imprecisoes-parecer-ia.md` |
