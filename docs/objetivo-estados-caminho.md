...
| ⚠ A.19 | **Normalizar a numeração de página no RAG.** Medido em 11/09/2026: **oito de 27 artigos testáveis tinham `page` fora da faixa que o próprio `abnt` declara** (37 ao todo; dez não declaram faixa). ⚠ **A inversão que isso revelou: em quatro citações dos pareceres o modelo estava certo e a base errada.** Saaty (2003, p. 85) nas execuções 2 e 4, Saaty (1987, p. 165) e Forman e Peniwati (1998, p. 169) na 1 caem dentro da faixa real e fora do `page` registrado. **Os dois últimos eram o que o documento chamava de "erro de vizinhança", categoria que se revelou invenção nossa.** **✅ Parte 1 fechada em `23afe43`:** sete artigos convertidos de numeração relativa para a do periódico, 54 campos `page`, nada mais tocado. A conferência de faixa caiu de oito artigos para **um**. Pré-requisito verificado antes: cada um é internamente consistente; e a fórmula foi validada contra o PDF do Saaty (2003), cuja página relativa 2 traz impresso 86. ⚠ **Parte 2 pendente: o `bozoki2010_ipc`.** `page: 2256/2257` num artigo de 318-333, e **o `locator_id` também está errado**: usa "Theorem 1" para a claim de unicidade, quando o Teorema 1 da p. 321 é sobre logconvexidade de λmax. O conteúdo das seis claims corresponde ao artigo, então é falha de preenchimento, **mas a correção é reancoragem por leitura, com o PDF aberto**. **Commit próprio:** critério mecânico e critério de leitura não devem compartilhar diff, porque o segundo contamina a conferência do primeiro. **Taxa refeita à mão para a execução 1:** 5 em claim, 2 na faixa sem claim, 11 divergentes. **Cai de 72% para 61%.** ⚠ **As taxas seguem provisórias até o `verify-citations.mjs` incorporar o segundo teste**, comparar contra a faixa do `abnt` além do `page` da claim. Evidência em `docs/imprecisoes-parecer-ia.md` **Parte 2 iniciada em 11/09/2026: três das dez âncoras localizadas no PDF.** A claim de unicidade traz **três erros numa só**: o enunciado real é o **Teorema 2, na página 324**, e diz "problem **(4)**", não (5); o RAG registra p. 2257, Theorem 1 e problema (5). As outras duas localizadas são da **Seção 2.2 "Graph representation", p. 321**, contra "Section II Methodology" e "Section II" no RAG. ⚠ **Confirma a leitura de A.16:** aqui o `verbatim_quote` era o campo fiel, porque preservava "corresponding to the incomplete pairwise comparison matrix" que o `evidence.quote` truncava. **A troca de A.17 fez o modelo receber, nesta claim, o texto mais curto E com o número de problema errado.** ⚠ **Nenhuma das dez pode ser corrigida por deslocamento aritmético**: a numeração do RAG não corresponde à do artigo em nenhum dos três casos verificados. **Faltam sete**, espalhadas entre as páginas 320 e 333 das 33 do artigo, e localizá-las é leitura dedicada |
