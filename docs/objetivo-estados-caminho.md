...
- Buscar o início de um bloco a remover por um delimitador que **não é único**.
  ...
  **É a mesma família das âncoras de texto: delimitador que não é único não
  ...
  ⚠ **O que detectou foi o `tsc`; a revisão do diff não detectou.** O diff completo
  **mostrava** as 200 linhas apagadas — a informação estava lá. **Eu revisei e não
  vi o excesso**, e o `npx tsc --noEmit` acusou oito nomes inexistentes na hora.
  **A falha foi da revisão, não da visibilidade.**

  ⚠ **E a implicação inversa importa mais: o `tsc` NÃO substitui a revisão do diff.**
  Remover código válido e **não referenciado** passa pela compilação sem ruído — foi
  exatamente o caso de **A.6**, em que um arquivo órfão de 315 linhas saiu e o `tsc`
  não acusou nada, corretamente. **Conferir se a remoção respeitou o escopo é
  trabalho da revisão do diff; o typecheck só pega o que quebra referência.**
...
- Perguntar QUAL antes de confirmar QUANTOS. Uma investigação recente examinou
  `/api/validate-external` e o `ExternalValidation.tsx` e voltou com resposta
  afirmativa, sob o rótulo errado. **A pergunta que a abriu foi "qual validação a
  rota deixa de bloquear, e se é a mesma da Tabela 7", e ela pressupunha um
  validador único.** Havia dois, de objetos diferentes: o do **texto do parecer**
  (`validateReviewOutput`) e o da **validação computacional externa** contra a
  AhpAnpLib. Quem executou foi ao segundo e respondeu "sim" — **e a resposta veio
  ancorada em arquivo, função e linha, o que a fez parecer verificada. Ela era
  verificada, sobre o objeto errado.**

  **A regra: antes de perguntar QUAL, confirmar QUANTOS.** Uma pergunta que
  pressupõe objeto único faz a verificação **confirmar a premissa em vez de
  testá-la**, e o localizador correto é justamente o que disfarça.

  ⚠ **Não é caso isolado: é a mesma família de
  `docs/imprecisoes-parecer-ia.md`, "O mesmo fenômeno em cinco níveis: a lacuna
  preenchida com o que tinha a forma esperada".** As três propriedades estão lá e
  não se repetem aqui: a resposta **não sinalizou a lacuna**, tinha **a forma
  esperada**, e o que a pegaria é **confrontar com fonte independente do próprio
  artefato** — neste caso, contar os validadores antes de escolher um.

  ⚠ **E o erro NÃO chegou aos documentos, verificado em 13/09/2026 em `39f3625`.**
  Busca em `docs/` por `A.27`, `validate-external`, `ExternalValidation`,
  `AhpAnpLib`, `all_valid`, `matrixSource`, `Tabela 7` e `T7`: **nenhuma linha
  atribui a investigação da validação externa a A.27**, e **nenhuma promoção de
  A.27 está registrada** — as duas menções a `A.27` são a lista de itens
  independentes da remoção do IPC, na linha de A.23, e a própria linha de A.27, que
  cita `ai-reviewer/route.ts` e não a rota externa. **O rótulo errado existiu fora
  dos documentos versionados.** Registrar isso importa porque a decisão derivada
  dele — promover A.27 — foi tomada sobre o item errado e não vale.
...
⚠ **SUPERADO em 13/09/2026, por A.24.** O parágrafo abaixo dizia que a garantia era
local e manual, e deixou de valer: `.github/workflows/ci.yml` existe desde `a96aa34`,
roda em `push` e `pull_request`, e **derruba o job se o censo falhar** — o passo
`Testes` executa `npm test`, que inclui os doze testes do censo. A primeira execução
saiu verde, com 91 testes em Node 24.x. **O que permanece verdadeiro:** o censo não
bloqueia *merge* por si, porque proteção de branch é configuração do GitHub e está
fora do escopo de A.24.

**Texto anterior, mantido como registro do raciocínio:** *"Correção de 11/09/2026: o
censo NÃO quebra o build. Não existe GitHub Actions neste repositório — `.github/`
contém só `copilot-instructions.md`. A garantia é local e manual, dependente de
alguém rodar `npm test`."*
...
O teste **roda a rota e confere contra a Tabela 12**: reproduz os valores publicados
no caso de referência, dentro das tolerâncias verificadas. **Ele não compara as duas
implementações entre si**, e por isso não estabelece equivalência — para isso o teste
teria de confrontar as duas saídas diretamente.

**O que está medido é que as duas DIFEREM**, em 12/09/2026: a cópia da rota tem
**seis clamps `EPS_GUARD = 1e-12`** e o `synthesizeBOCR` **nenhum**.

| Onde | Quantos | O que o clamp protege | Sobre o que |
|---|---|---|---|
| 307 | 1 | denominador da Eq. 9 | `(sc·C) + (sr·R)`, que mistura *rescaling weights* e prioridades |
| 316 e 317 | 4 | base de `Math.pow`, Eq. 7 | as prioridades B, O, C, R, uma a uma |
| 321 | 1 | denominador da Eq. 7 unitária | o produto `C·R` |

⚠ **Diferença de implementação NÃO é divergência numérica demonstrada.** Os clamps
protegem coisas diferentes, e **peso estratégico nulo e prioridade de mérito nula não
são o mesmo caso**: uma entrada que dispara o clamp de 316 pode não disparar o de
307. **Para afirmar divergência é preciso registrar a entrada concreta, o método de
síntese e as duas saídas, incluindo eventual exceção.** Enquanto isso não existir, o
que se afirma é só a diferença de código.

**Unificar continua sendo a correção.** Nenhum dos doze testes verifica unicidade
da síntese. 10 306
...
| A.10 | **Auditar as citações e afirmações do prompt contra o RAG e contra o sistema. ✅ 1º de 3 commits em `2b353b3`:** três localizadores corrigidos. A execução 2 confirmou que a correção propagou: as duas citações de Wijnmalen saíram corretas e nenhuma outra melhorou sozinha. **2º commit:** remover o dado real dos exemplos de parágrafo, incluindo a frase "os 12 respondentes apresentam CRs entre 1,1% e 9,6%", que é a imprecisão 1 ensinada como modelo de redação. **✅ 2º commit em `ca36539`:** o payload passou a informar N = 12 em todas as matrizes. As três predições registradas antes da execução se confirmaram: a imprecisão 2 desapareceu, Escobar passou a ser aplicado com N=12, e a faixa dos CRs permaneceu como controle. **Classe E confirmada como fato medido, ao custo de quatro linhas.** ⚠ **O ITEM DA FAIXA CONTINUA ABERTO, medido em 12/09/2026 em `f9cbf94`:** `app/api/ai-reviewer/system-prompt.ts:323` ainda traz *"Os 12 respondentes individuais apresentam CRs entre 1,1% e 9,6%, todos abaixo do limiar"* dentro do exemplo de parágrafo. **O rótulo "2º commit" foi consumido por `ca36539`, que fez o N por matriz — mudança diferente** —, e a remoção do dado real dos exemplos ficou sem slot, parecendo feita. ⚠ **COORDENAR COM A.12, porque são DUAS fontes entregando o mesmo erro ao modelo:** o exemplo do prompt (esta tarefa) e os CRs em cache que chegam pelo payload (A.12 e A.25). **Corrigir só uma não autoriza esperar que a afirmação desapareça**, e a execução 7 mostra a afirmação de pé com as duas presentes. **3º commit:** (a) auditar as 61 combinações de autor e ano; (b) auditar o restante do payload, onde `- ✅ Validação externa com pyAHP` faz o parecer afirmar biblioteca que o sistema não usa. Equações e limiares já auditados. Evidência em `docs/imprecisoes-parecer-ia.md` |
...
| A.23 | **Corrigir o tratamento de empates. ⚠ ACHADO F04, e é o mais consequente para o artigo.** Com duas alternativas idênticas, escores **0 e 0**, o sistema declara **A1 vencedora com 100% de concordância** entre os cinco métodos de síntese. **A ordem de entrada resolve o que o modelo não resolveu.** ⚠ **É pior que o `robustnessLevel` já registrado:** não é só confundir concordância com robustez, é **declarar consenso onde não existe diferença**. Cinco métodos concordando sobre dois zeros não é convergência, é ausência de informação. **Toca exatamente o construto que a seção 1.6 identifica como linha de contribuição**, e é **independente da remoção do IPC**, como também são A.25, A.27 e A.28 (correção: não é o único). **Sai à frente de A.16 e A.18**, porque produz resultado errado hoje ⚠ **PERGUNTA aberta, registrada em 12/09/2026, e é pergunta, não relação estabelecida:** a rota tem **seis clamps `EPS_GUARD = 1e-12`** nas fórmulas de síntese, e o `synthesizeBOCR` do motor não tem nenhum. **Os escores 0 e 0 deste achado são a região onde um clamp de denominador atua** — mas **dois escores zero não demonstram, sozinhos, que a causa é o clamp.** O que responderia: montar a entrada que produz o empate e comparar **as duas implementações lado a lado**, registrando as duas saídas e eventual exceção. Se o clamp participar, F04 e a duplicação da seção 2.4 se tocam; se não, são independentes. |
| ✅ A.24 | **Criar o workflow do GitHub Actions.** Sem ele, **nenhum teste protege a entrega**, e o censo é garantia declarada sem mecanismo: depende de alguém rodar `npm test` antes do push. `.github/` existe e contém só `copilot-instructions.md`. **É pequeno** e resolve o problema de fundo que o relatório aponta. Mínimo: `npx tsc --noEmit`, `npm test` e `npm run build` em cada push ⚠ **FECHADA em 13/09/2026, em `a96aa34`.** Um arquivo, `.github/workflows/ci.yml`, 63 linhas, e **nada fora dele**: nenhum código de produção, nenhum teste, nenhum `package.json`, `tsconfig` ou configuração de build. **Gatilho em `push` e `pull_request`**, um job em `ubuntu-latest`, e os três comandos em passos separados na ordem **typecheck → build → teste**. Sem `continue-on-error`, `|| true`, `--passWithNoTests`, `set +e` nem `if: always()` — conferido no arquivo, onde as únicas ocorrências desses termos estão no comentário que documenta a ausência deles. **Execução remota verde:** https://github.com/pedropalma1179/ahp-simple/actions/runs/34735840052 — evento `push`, conclusão `success`, os sete passos `completed/success`, 03:34:53Z a 03:37:47Z. ⚠ **`node-version: 24.x` porque é o que a Vercel usa em produção** (`nodeVersion` do projeto), pela regra de que CI e deploy divergentes produzem verde no CI e falha no deploy. O repositório **não declara versão**: sem `engines`, sem `.nvmrc`, sem `.node-version` — e a versão não foi inferida de `lockfileVersion`, das actions nem da máquina local. ⚠ **A divergência de versão foi medida e fechada:** a linha de base local saiu em **Node 25.9.0** com 91 testes, e o runner, em **24.x**, reporta **`Test Suites: 5 passed, 5 total` e `Tests: 91 passed, 91 total`**, em 2,009 s contra 0,753 s locais. **Mesmo total nas duas versões.** ⚠ **Limitação do método de conferência, registrada porque muda quem mediu o quê:** o download do log exige direito de administrador — `GET .../logs` devolve **403 "Must have admin rights to Repository"** —, `gh` não está instalado nesta máquina, e a saída do check-run vem vazia. **O número do runner veio do log colado pelo pesquisador**, não de leitura automatizada. O que a API confirma por si é que o passo `Testes` saiu `success`, isto é, `npm test` exitou 0 em 24.x. ⚠ **ENTREGA EXECUÇÃO AUTOMÁTICA DA COBERTURA EXISTENTE, E ELA É PARCIAL:** a suíte protege o cálculo e **não vê o contrato do parecer nem a UI**. **Os testes dos contratos do parecer e da UI acompanham as respectivas correções**, cada um na tarefa que altera o que ele verifica. Registrado para que A.24 não se torne garantia declarada, que é o defeito que ela corrige ⚠ **CI confirmado no HEAD, em 13/09/2026:** consulta autenticada devolveu `completed` e `success` para o SHA completo de `0b17115`, na execução https://github.com/pedropalma1179/ahp-simple/actions/runs/34784737577. **O verde de `a96aa34` permanece como registro histórico da criação do workflow, e não é ele que sustenta a afirmação sobre o HEAD.** ⚠ **Limitação do método, mantida como registro:** duas tentativas anteriores, sem autenticação, devolveram **403 por limite de taxa**, e 403 não é ausência de execução |
...
| A.27 | **⚠ F06: o parecer não é impedido de apresentar números inválidos.** O validador aceita como `isValid=true` texto com escores fictícios escritos com **vírgula decimal**; com ponto, identifica números desconhecidos **só como avisos**. E quando retorna inválido por referência proibida, **a rota não bloqueia a apresentação**: a interface usa o sucesso e o texto. Fontes: `ai-reviewer/route.ts` linhas 1491 e 1657. **Aceite:** falha de verificação bloqueia ou põe em quarentena visível ⚠ **LOCALIZADORES ATUALIZADOS, conferidos em 13/09/2026 em `39f3625`:** as linhas 1491 e 1657 acima **não valem mais** — os commits de A.21 e A.32 removeram linhas do arquivo. Hoje: `validateReviewOutput` é **definido em `:1410`** e **chamado em `:1642`**; o retorno é montado em **`:1665`**. ⚠ **E são DOIS validadores, não um:** o `validateReviewOutput` chama o `validateCitationsAgainstWhitelist` (`lib/rag/citation-whitelist.ts:264`) em **`:1516`**. ⚠ **Medido: a verificação não impede NEM a devolução NEM a exibição.** Com `isValid: false`, as linhas 1644 a 1650 fazem apenas `console.error` e `console.warn`, e `:1665` devolve `NextResponse.json({ success: true, … review, … })` — o `success: true` é **literal**, o texto vai inteiro, e o veredito de validação viaja como campo que o chamador pode ignorar. ⚠ **PRÓXIMA TAREFA, decidido em 13/09/2026, por prioridade de correção.** O sistema já identifica parte das falhas e ainda assim devolve o parecer como sucesso, e corrigir recuperação e contexto não elimina esse defeito de apresentação. ⚠ **A proposta anterior, de posicionar logo depois de A.12, fica superada, e não foi identificada dependência técnica de A.12:** A.12 trata da distribuição categórica de status que entra no payload, e este item trata do que a rota devolve e do que a interface apresenta quando a verificação reprova. A promoção anterior para junto de A.23 já havia sido invalidada, porque a investigação que a fundamentou tratou da validação computacional externa, que é **A.28**; ver a entrada em 0.4. ⚠ **Fase 1 medida e registrada em 13/09/2026, em `docs/imprecisoes-parecer-ia.md`, seção de F06.** A consequência da reprovação **não está decidida**, e a correção não começa antes disso. ⚠ **EIXO 1 IMPLEMENTADO em 13/09/2026, em dois commits, `7394cef` (extração mecânica) e `0f99fad` (as três mudanças).** O separador decimal passou a ser reconhecido, o escore incompatível com os dados passou a **reprovar** em vez de avisar, e existe estado `inconclusivo` com motivo para quando há afirmação numérica e nenhuma referência válida. ⚠ **LOCALIZADORES MUDARAM DE ARQUIVO:** o validador saiu da rota e está em **`lib/ai-reviewer/validate-review.ts:62`**, com `ValidationResult` em **`:33`** e o estado computado em **`:186`**; o contrato da requisição e `getValidFinalScores` estão em **`lib/ai-reviewer/review-request.ts:37`** e **`:141`**; na rota, a chamada é **`:1365`** e a resposta **`:1389`**. ⚠ **A.27 CONTINUA ABERTA, e isto é medido, não ressalva de estilo:** nos seis casos exercitados pelo handler depois de `0f99fad`, a resposta seguiu com `success: true`, `nota: A` e `veredicto: ACEITO`, e o texto voltou inteiro **inclusive nos dois que agora reprovam**. O que existe é **o resultado da verificação disponível e não consultado**. **O eixo 2, contrato da API e apresentação, é o que torna a verificação efetiva**, e só ele fecha o aceite. |
| A.28 | **⚠ F07: cenários sobrescrevem o resultado-base.** O cálculo completo e um cenário com R01 excluído **gravam no mesmo documento** `calculations/{projectId}`, e a constante de versão do motor não é persistida como identidade da execução. Fonte: `calculate/route.ts` linha 1242. **E a validação externa depende de um backend Python que não está neste repositório**, com um fallback legado que reconstrói a matriz a partir dos pesos — o que **não equivale a validar os julgamentos originais**. **Aceite:** simulação não altera o resultado-base; cada afirmação remete à execução usada ⚠ **SUBITEM: validação computacional externa. É objeto DISTINTO do F07 acima**, que trata de cenários sobrescrevendo o resultado-base; esta linha acumula os dois porque a validação externa não tem item próprio. **Escopo do subitem:** `app/api/validate-external/route.ts`, `components/ExternalValidation.tsx` e o microserviço com AhpAnpLib, que **não está neste repositório**. Apura (a) o bloco rotulado como citação para dissertação e de que condições ele depende; (b) o que a seleção entre matrizes persistidas e reconstruídas demonstra e o que não demonstra; (c) o que o veredito da comparação governa na interface; e (d) a proveniência da Tabela 7. **Especificação detalhada, com os nove estados de validação, as decisões pendentes e as restrições: `docs/validacao-externa-especificacao.md`.** ⚠ **Os nove estados são requisitos da implementação futura, não resultados verificados.** ⚠ **STATUS, 13/09/2026: diagnóstico da validação externa concluído no commit examinado; decisões da Fase 2 pendentes.** O diagnóstico medido está em `docs/imprecisoes-parecer-ia.md`. ⚠ **Uma pergunta do diagnóstico ficou CONDICIONAL e não fechada:** se os dois lados são efetivamente comparados depende do que o microserviço faz, e ele está fora do repositório. ⚠ **SEM POSIÇÃO NA FILA:** nada aqui a decide. **A.27 permanece F06**; sua prioridade está registrada na linha própria. ⚠ **A correção futura impede repetição e NÃO verifica retroativamente a Tabela 7** |
...
| A.30 | **⚠ F09: diagnosticar a falha da recuperação semântica. PRÉ-REQUISITO OPERACIONAL DE A.18.** Medido na execução 7, em `f9cbf94`: cinco consultas, cinco resultados vazios, cinco `[rag/semantic-retrieve] retrieval failed (silent failover): Unexpected end of JSON input`, e o parecer gerado com **zero chunks semânticos** sem nenhum sinal na saída. **O caminho estava ligado** — a linha de resumo só imprime quando `USE_RAG_SEMANTIC` é verdadeiro. ⚠ **A etapa que falha NÃO está localizada:** o `catch` de `lib/rag/semantic-retrieve.ts:47` envolve `embed(query, 'query')` na linha 45 e `querySimilar(vector, topK)` na 46, então a mensagem pode vir do processamento da resposta de qualquer uma das duas dependências. ⚠ **E `failedQueries` não conta falhas:** `route.ts:95` faz `results.filter(r => r.length === 0).length`, que **conta resultado vazio** e reúne consulta sem resultados com consulta com erro. **Escopo:** (1) localizar a etapa que falha, distinguindo Voyage de Upstash; (2) **separar erro de resultado vazio** na instrumentação, para que a contagem deixe de ser ambígua; (3) sinalizar **execução degradada nos metadados** do parecer, para que zero chunks deixe de ser silencioso. **Aceite:** a etapa nomeada com evidência; erro e vazio distinguíveis no log e nos metadados; e **comprovação de que chunks são recuperados E incluídos no contexto**, conferida no contexto enviado, não só na contagem. ⚠ **Não é a reingestão**, que é A.18 ⚠ **Fase 1 medida e registrada em 14/09/2026, em `docs/imprecisoes-parecer-ia.md`.** **A etapa que falha está localizada neste ambiente, e é a consulta ao índice:** nas cinco consultas o `embed` devolveu vetor de 1024 dimensões e `querySimilar` lançou `SyntaxError` com a mesma mensagem do log histórico, e duas requisições de leitura independentes ao endpoint configurado devolveram **404 com corpo vazio**. ⚠ **Sem captura da própria requisição do SDK, isso não demonstra que ele recebeu aquelas respostas**, e o mecanismo fica registrado como compatível e reproduzível. ⚠ **Isso localiza a etapa no ensaio de 14/09/2026 e NÃO estabelece a origem da falha da execução 7**, que rodou em produção; a comparação entre os dois ambientes não foi medida, e a razão do 404 também não. **Medido junto:** vazio sem exceção e exceção produzem `failedQueries` idêntico, erro de embedding e erro de consulta produzem avisos idênticos, e recuperação desligada, índice sem correspondência, falha de embedding e falha de consulta entregam ao modelo **o mesmo prompt byte a byte**. A correção deve distinguir erro, resultado vazio e recuperação desabilitada nos registros e metadados. A identidade dos prompts é um achado desta rodada; informar essas condições ao modelo depende de decisão específica. A correção não começa antes das decisões pendentes ⚠ **Apuração do 404 em 14/09/2026:** **causa não identificada**, com cinco hipóteses vivas: recurso removido ou suspenso, URL configurada apontando para outro recurso, segmento de caminho na URL configurada, 404 ocultando proibição de acesso na forma que a RFC 9110 admite, e 404 vindo de intermediário na rede da Fase 1. Cada uma vem com o que a tornaria falsa, na seção de F09 de `docs/imprecisoes-parecer-ia.md`. **Medido, e fecha o mecanismo sem fechar a causa:** o ensaio controlado demonstrou que cinco status diferentes, com corpo vazio, produzem a mesma mensagem, medido em 200, 401, 403, 404 e 500. **O log histórico isolado não identifica o status, não demonstra corpo vazio e não localiza a etapa. O 404 com corpo vazio foi observado nas sondas independentes da máquina Windows.** **Medido também:** corpo não vazio produz a mesma frase, em `[`, `{"a":`, `[1,` e `{"result":`, e `{` sozinho produz outra, então a mensagem não distingue corpo vazio de corpo truncado. **Refutada por medição:** espaço nas bordas da URL, que produziria `TypeError: Failed to parse URL` e nenhuma requisição. **Também medido:** o deploy da execução 7 **é identificável**, `dpl_orQbNxPZQP8htwtZPvdUZ5gPhSbP`, produção, criado em `2026-09-13T01:51:56Z` e substituído em `03:12:13Z`. **Ambiente apurado: nenhum dos dois**, contêiner remoto Linux sem as três variáveis e com egresso bloqueado para `upstash.io`, e **o ensaio do SDK rodou contra servidor local, então caracteriza o cliente e NÃO conclui a apuração do endpoint configurado**; a sonda contra o endereço configurado, a discriminação do item 4, o console e as variáveis de produção ficam **não apurados**, e quatro informações da máquina do pesquisador abrem a rodada seguinte ⚠ **Restauração e recorrência em 14/09/2026, SAÍDA FORNECIDA PELO PESQUISADOR, não medida nesta sessão:** console mostrando o índice `ahp-bocr-rag` em **`HIBERNATED`**, AWS `us-east-1`; restauração manual entre os dois ensaios, com horário exato não registrado; e o mesmo ensaio local passando de **404 com corpo vazio às 19:24:25Z**, nas sondas e na chamada real do SDK, para **200 com cinco chunks às 19:40:23Z**, com a rota inexistente devolvendo **404 com JSON válido**, e as respostas diferiram nos dois estados sem identificar universalmente a origem de um 404. Tela posterior: 473 registros, 1024 dimensões, COSINE. **Nova hibernação relatada poucos minutos depois, sem captura nem ensaio dela.** ⚠ **NÃO houve predição registrada antes da restauração.** **Alcance:** restauração **associada** à recuperação operacional no ensaio local; disponibilidade contínua **não demonstrada**; execução 7 **não explicada**; inclusão dos chunks no contexto **não verificada**, e a flag estava ausente nos dois ensaios. **H3, segmento de caminho, SAI**, porque a URL do ensaio tem zero segmentos; H1 é a hipótese que o material sustenta; H2 e H5 ficam enfraquecidas e não eliminadas; H4 não foi testada. Registro em `docs/imprecisoes-parecer-ia.md`, seção de F09 ⚠ **Instrumentação implementada em 14/09/2026, em `3be55b9`, e A.30 SEGUE ABERTA.** **Arquitetura:** `Requester` próprio pela extensão pública do SDK e um `Index` por consulta, restritos ao caminho de busca, com `upsertChunk` e `deleteAll` intactos no singleton; e, na Voyage, envoltório **só do `fetch` injetado**, devolvendo a resposta intacta, sem ler nem clonar. **O `fetch` global não é substituído em produção.** **Contrato versão 2:** etapa, estado do embedding e estado da consulta separados; falha de embedding deixa a consulta **não executada**, sem tentativa fictícia; status **ausente** quando não houve resposta e **preservado** quando houve, inclusive com parse falhando depois; bytes são os efetivamente lidos **(afirmação SUPERADA, ver o próximo marcador)**. **`withResults`, `emptyOk` e `errored` são disjuntas e somam `queriesRan`**, e **`failedQueries` fica como compatibilidade derivada de `errored`, com significado MUDADO**: antes reunia vazio legítimo com erro. **Comparação medida, não suposta:** dez casos com o SDK real dos dois lados, requisição, repetições, intervalos e resultado equivalentes, mais a caracterização de `50c80f5` passando inteira, e o `system` e os `messages` completos conferidos contra execução do handler de `d658e50` em checkout separado, idênticos nos quatro casos. **Medido:** tsc 0, 11 suítes e 146 testes, build de 17 páginas. ⚠ **Fora deste eixo, e por isso A.30 não fecha:** apresentação na interface, tratamento da geração degradada, verificação operacional contra o índice real, e a identidade dos prompts sem chunks, que segue comportamento atual e cuja alteração exige predição registrada antes ⚠ **Cinco correções da instrumentação em 15/09/2026, em `d66ca2a`, e A.30 SEGUE ABERTA.** Cada uma reproduz um contraexemplo **medido sobre `3be55b9`**, e as regressões de `lib/__tests__/rag-diagnostico-regressao.test.ts` reprovaram **13 dos 14** casos naquele commit, o décimo quarto sendo controle de caminho exitoso; dois casos acrescentados depois varrem o **corpo que a rota devolve**, e mostram que em `3be55b9` o token e a URL saíam nas **cinco** consultas dentro de `metadata.knowledgeBase.semantic.queries`, ou seja **na resposta HTTP e não só no log**. **(1) Sanitização que não sanitizava:** URL e token de um `TypeError` chegavam inteiros ao diagnóstico e ao `console.warn`, objeto lançado era serializado e `name` adulterado saía intacto; entra `descreverFalha`, que **não recebe o valor lançado**, e `nome` **sai do contrato**. **(2) Tentativas das duas etapas no mesmo array:** com o índice sem token o aviso dizia `falhou na etapa querySimilar (status 200, 1 tentativa(s))` com **zero** requisições ao índice, e o 200 era da Voyage; `tentativas` vira `tentativasEmbed` e `tentativasConsulta`. **(3) Ausência de resposta classificada como transporte:** são **três** condições e não duas, entra `anterior_a_chamada`, e a tentativa relevante é a **última** e não qualquer uma, que o `some` confundia. **(4) `bytes` sai do contrato:** um byte `0xFF` virava **três**, porque `res.text()` decodifica, e o campo media o texto decodificado sob nome de bytes da resposta; **perde-se a distinção entre leitura que não conclui e leitura vazia**, e a perda é consciente. **(5) Telemetria incondicional:** com `UPSTASH_DISABLE_TELEMETRY` definida a referência enviava **zero** cabeçalhos e o candidato enviava **três**; a comparação anterior era exaustiva em dez dimensões e rodava **num estado só**, e **cobertura de dimensões não é cobertura de estados**. **Medido neste commit:** tsc 0, **12 suítes e 164 testes**, build gerando **17 páginas estáticas** e listando **18 rotas** **(dois números diferentes: uma redação anterior desta linha dizia `17 rotas`, juntando o valor de um com o nome do outro; o `17 páginas` das notas antigas está correto)**, Linux x86_64, Node v22.22.2, npm 10.9.7 **(ambiente CORRIGIDO depois de `9c21288`, que trazia um ambiente HERDADO da nota do `CLAUDE.md` em vez de medido)**; nenhum teste removido, quatro com expectativa alterada e a razão de cada um registrada. **Equivalência de transporte remedida**, agora nos dois estados da telemetria, e os quatro resumos de contexto de `d658e50` **continuam batendo byte a byte**. ⚠ **O temporizador de tempo limite da Voyage foi ALCANÇADO pelas regressões e NÃO foi corrigido**, por estar fora do escopo: as que o alcançam usam temporizador falso, e nada se afirma sobre o efeito fora do ensaio. Registro em `docs/imprecisoes-parecer-ia.md`, seção de F09 |
| A.31 | **⚠ F11: corrigir a seleção temática da recuperação estática.** Medido em 12/09/2026: `getRefsByTopic('consistência')` devolve **93 das 138 claims** e as injeta em `route.ts:1147` sob o título "Referências sobre Consistência e Validação de Dados"; `getRefsByTopic('BOCR')` devolve 28 e injeta em `:1155`; as quatro seções estáticas entregam **100 claims distintas de 138**. **Entre as 93 está a claim do Saiyed sobre poder do CEO e viés cognitivo**, que não trata de consistência — e ela entra **duas vezes**, porque também casa 'BOCR'. ⚠ **NÃO é tarefa de excluir o Saiyed, nem de reduzir a quantidade de claims por si só.** O defeito é **a regra que o inclui** sob esses dois rótulos: é autorização por rótulo, porque o cabeçalho afirma pertinência que a claim não tem. **Escopo:** identificar **quais campos e termos** produzem o encaixe — o filtro casa `topic`, `context` e alias —, tornar o **critério de seleção explícito**, e corrigir o mecanismo **preservando os artigos na base**. **Aceite:** critério de seleção declarado no código; **conferência de pertinência das claims incluídas e de preservação das necessárias**, caso a caso nas seções afetadas; **duplicações medidas** entre seções; e tamanho do contexto final medido antes e depois. **O resultado esperado não é menos claims: é claim sob rótulo que corresponde** |
...
| A.33 | **⚠ Passar `evidence.page` para o contexto injetado. É o SEGUNDO caso medido da QUARTA superfície de desancoragem — a interface entre prompt e payload —, e o mais barato de fechar.** **Medido em 12/09/2026, no parecer da execução 7:** o parecer produziu **21 localizadores** e o contexto recuperado contém **três**. Das quatro seções estáticas, as **135 referências injetadas** têm os campos `id`, `citation`, `topic`, `rule`, `context`, `weight` — **zero mencionam página** —, e `ragThresholds`, `ragFormulas` e `ragBenchmarks` dão **zero** também. As únicas páginas que chegam ao modelo são as **12 do `system-prompt.ts`**, que são **três pares distintos**: Saaty (1977, p. 248), Wijnmalen (2007, p. 899 e p. 903) e Forman e Peniwati (1998, p. 167). ⚠ **E o prompt EXIGE a página:** o item 7 da linha 500 manda incluir o quote *"com a página (quando disponível)"*. **É a classe E na mesma forma da DIRETRIZ 1:** instrução que exige fato que o payload não fornece, e o modelo preenche a lacuna para poder obedecer. **O dado existe e não é passado adiante:** das **138** key_claims, **134 têm `evidence.page` numérico**; as 4 sem página são 2 do `liang2022_ahp_undesirable` e 2 do `saaty1990_howtomake`. **Escopo, e é pequeno:** acrescentar o campo de página ao `ReferenceDoc` (`app/api/ai-reviewer/knowledge.ts:23`), preenchê-lo em `claimToRef` a partir de `claim.evidence.page`, e passá-lo nos quatro templates de `route.ts` (`:1147`, `:1155`, `:1163`, `:1171`). **Tratar a ausência explicitamente**, com a convenção já fixada em `referencia-cr-individuais.md`: **campo vazio para indefinido, nunca zero** — e, aqui, nunca uma página inventada para preencher a forma. ⚠ **ALTERA O QUE O MODELO RECEBE, logo EXIGE PREDIÇÃO ESCRITA ANTES**, em `docs/imprecisoes-parecer-ia.md`, com o caso negativo nomeado. **Sugestão de predição, a confirmar antes de executar:** as páginas dos artigos cujas claims forem citadas passam a coincidir com `evidence.page`, e o caso do **Forman e Peniwati** é o teste mais forte — hoje o prompt ensina p. 167, a base tem claims em 166, 167 e 168, **e o parecer escreveu p. 170**. **Caso negativo nomeado:** o parecer continuar produzindo página fora da faixa do artigo para claim cuja página foi injetada; se isso ocorrer, a lacuna de contexto não era a causa. **Controle:** a faixa dos CRs individuais, que esta tarefa não toca. ⚠ **O que esta tarefa NÃO resolve, e não deve prometer:** (a) as citações **sem** localizador, 18 no mesmo parecer; (b) as páginas que o modelo acerta **sem** origem no contexto — quatro na execução 7 —, que ficam sem explicação medida; (c) a **fidelidade da transcrição** e a **sustentação da afirmação**, que são as outras duas coisas que a seção 7 do `CLAUDE.md` manda manter separadas. ⚠ **Depende de A.16** para o conteúdo do campo citado, e **não depende de A.18**: a injeção estática não passa pelo índice semântico ⚠ **ESCOPO SUBSTITUÍDO em 15/09/2026, e a proposta acima fica SUPERADA.** A decisão vigente é **paráfrase com fonte identificada, sem páginas e sem obrigatoriedade de citação direta**, e não mais acrescentar `evidence.page` ao contexto. **Predição registrada ANTES da primeira alteração, em `0bf9927`**, com quatro casos negativos e controle. **Implementado em `50f0794` e `b9a4488`.** **Medido:** as cinco instruções normativas saíram; as ocorrências de exemplo eram **DOZE e não dez**, e as duas a mais estavam no EXEMPLO INTEGRAL, uma delas na metade rotulada CORRETO; depois de editar, **zero `p. N` no prompt** e zero `verbatim` nas instruções. **As duas referências normativas erradas saíram e NÃO foram substituídas por outro número**, porque a norma não foi consultada nesta sessão. `formatSemanticChunks` parou de interpolar a página, e **`ChunkMetadata.page` e `evidence.page` PERMANECEM** para auditoria. **Testes de montagem, sete casos: zero páginas no `system` e nos `messages` nos três cenários**, com o chunk do ensaio carregando `page: 248` de propósito, então o zero é retirada e não ausência. **Triagem léxica com os três controles, o esperado declarado antes:** transcrição sem aspas dá corrida 15 e sinaliza, as duas paráfrases dão 1 e não sinalizam, e **o terceiro controle fica registrado como LIMITE**, porque a triagem é léxica e não vê inversão de sentido. ⚠ **Verbatim chega ao modelo por QUATRO caminhos, e isso é DESCRIÇÃO do que o contexto entrega, não hipótese de causa.** ⚠ **INFERÊNCIA RETIRADA em 15/09/2026:** a redação anterior chamava isso de maior ameaça à predição e de explicação mais provável de uma refutação, **sem medição**. **Manter o texto original no contexto não contradiz pedir paráfrase na saída**, porque o modelo precisa receber a evidência para reformulá-la, e o rótulo `Verbatim` **descreve a origem do material**. **Os trechos originais são PRESERVADOS, e retirá-los não está autorizado.** ⚠ **ETAPA 4 NÃO EXECUTADA: a mudança está implementada e a predição permanece não testada.** As quatro credenciais estão ausentes neste ambiente, o egresso funciona e `api.anthropic.com` responde 401; sem credencial não há parecer gerado. **As colunas 1 e 2 da conferência já estão automáticas** em `lib/rag/conferencia-sustentacao.ts`, e só a coluna 3 depende de leitura da publicação. Registro em `docs/imprecisoes-parecer-ia.md` ⚠ **Três correções no instrumento de conferência em 15/09/2026, em `89f657e` e `b1c9d4a`, sobre `1444892`.** As regressões reprovam **13 de 13** naquele commit. **(1) Citações permitidas desapareciam:** `(Wijnmalen, 2007)`, `(Forman e Peniwati, 1998)` e `Dodevska et al. (2023)` davam **zero entradas**. ⚠ **A causa NÃO era a que o contraexemplo sugeria:** o padrão parentético funcionava, e quem quebrava era o **separador de frases**, que cortava `et al.` ao meio em `Dodevska et` e `al. (2023)`. **Onze formas cobertas**, inventariadas por leitura do prompt, **mais quatro declaradas fora**, com exemplo e razão. **(2) Sobrenome tratado como identidade:** para Saaty (1977) entravam um exemplo de instrução e um trecho de Saaty (1980); a associação passa a ser por **obra e ano**, a origem da evidência vive no dado, e **exemplo do `system` nunca conta como evidência recuperada**. Associação ambígua se **registra como ambiguidade**, distinta de falta de sustentação, e nenhum candidato é escolhido. **(3) Divergência de outra obra contaminava:** a de `bozoki2010_ipc` tornava inconclusiva uma afirmação de **Bozoki (2013)**, e a triagem **não rodava**. A divergência passa a valer por **trecho**, e a triagem é calculada **sempre**, com **três estados**: `nao_sinalizada` fica reservado à triagem que rodou e não sinalizou. ⚠ **DUAS JUSTIFICATIVAS RETIRADAS**, do registro e do código: que seis palavras iguais seriam **raras**, sem medição de frequência, e que o intervalo de 1 a 15 provaria ausência de ajuste aos casos, quando mostra apenas que vários limiares separariam estes três exemplos. ⚠ **As 26 diferenças são resultado do CRITÉRIO aplicado**, igualdade exata após `trim`, e **normalizar espaço e aspas não demonstra que sejam substantivas de sentido**. ⚠ **ETAPA 4 DESTRAVADA quanto ao índice:** fixando as evidências por caso, C1 previamente e C3 com recuperação simulada vazia, a geração real avalia a **redação** sem depender da disponibilidade do índice nem de reingestão. ⚠ **Mas NÃO passa a depender só da credencial, e a redação anterior desta linha dava a entender que sim.** **Faltam três coisas:** o **conjunto efetivamente preparado por caso**, que está definido e não escrito; a **identidade dos trechos**, `articleId` mais `trechoId`, ainda não atribuída aos trechos fixados; e a **conferência das fontes contra as publicações**, que é a coluna 3 e não foi feita. Segue **não executada**. **Medido:** tsc 0, **16 suítes e 197 testes**, build com 17 páginas estáticas e 18 rotas ⚠ **Três correções na identidade da evidência em 15/09/2026, em `80420df`, sobre `8fed6bc`.** As regressões reprovam **15 de 20** naquele commit, e ⚠ **cinco são da espécie FRACA**, reprovando pela ausência do símbolo novo e não pelo comportamento: **teste assim não demonstra defeito**, e era o critério frouxo da rodada anterior. **(1) Autoria por coincidência parcial:** o `some` dava a obra por identificada com **qualquer autor em comum**, e `Forman e Peniwati (1998)` recebia evidência de **Forman e Gass (1998)** como identificada. Passa a ser comparação de **LISTA**: sem `et al.`, completa e na ordem; com `et al.`, os explicitados como **início** da lista. Autoria ausente na evidência **não permite a conferência**. **(2) Divergência por contenção textual:** `includes` nos dois sentidos, e **com cadeia vazia devolve verdadeiro sempre**, então campo vazio casava com qualquer trecho. Passa a vincular por `articleId` mais `trechoId`, **nos dois lados**, e identificador ausente ou vazio **não gera correspondência**. **(3) Inventário contradizia a execução:** `(SAATY, 1977)` e `(ABNT, 2023)` entravam como entradas interpretadas, e a **composta** produzia **UMA** obra, `autores: [Saaty]`, com o resto engolido como localizador. ⚠ **Esse é o pior desfecho: entrada com a obra errada PARECE cobertura.** As formas excluídas passam a ir para **revisão manual, com o fragmento preservado**, e cada exemplo tem teste exigindo **as duas coisas**, porque ausência sozinha não distingue tratamento de descarte. ⚠ **`FORMAS_FORA` caiu de quatro para três, e a redução é achado:** caixa alta ABNT e autor-entidade **não se distinguem pela forma**. As regressões passam a conferir **autores, ano e candidatos**, e não só a contagem: **produzir uma entrada não demonstra que a obra certa foi extraída**. **Medido:** tsc 0, **17 suítes e 217 testes**, build com 17 páginas estáticas e 18 rotas  ⚠ **Identidade insuficiente corrigida em `c0b4bcf`:** evidência recuperada sem `articleId` ou `trechoId` não vazio fica inconclusiva por identidade insuficiente, sem atribuição a A.16; a triagem permanece independente. Nove regressões novas falharam no resultado sobre `482d2d6`, e os dois controles novos com identidade completa já passavam. Medido: tsc 0, 17 suítes e 228 testes, build 0 com 17 páginas estáticas e 18 rotas. A.33 segue em revisão, com a predição não testada e a preparação dos casos e conferência das publicações pendentes ⚠ **Preparação de dados da etapa 4 em `b1111fb`, sobre `344d631`:** C1, C2 e C3 escritos em `docs/dados/a33-etapa4/casos.json`, sem geração. As 138 claims dos 36 artigos não possuem `id`; unicidade de ID de claim não estabelecida e nenhum `trechoId` inventado. **identificador não certifica o conteúdo**. Quatro seções com 135 ocorrências de 100 claims, mais 18 limiares, 36 fórmulas e 10 benchmarks, permanecem nos três casos. C1 fixa três chunks, acrescentando uma claim distinta; total 165 unidades, quatro conferidas e 161 pendentes nomeadas, sem reconciliação de A.16. Modelo e configuração preservados, incluindo a ausência de `temperature` na chamada; uma geração futura por caso, zero executadas. Faltam identidade recuperável, conferência das pendências e preparação da execução com credencial. A predição permanece não testada e `main` permanece em `33c1fdf`. ⚠ **Identidade derivada medida, sem adoção, em `5935579`, protocolo anterior `195e8a8`:** snapshot `745b496`, 101 claims e 64 adicionais, total 165. Composição explícita de artigo, localizador e SHA-256 completo de `evidence.quote`, sem normalização. Seis grupos indistinguíveis pela composição, 13 unidades; nove grupos de quotes byte a byte iguais, 22 unidades. O candidato não cobre as 165: 107 atendem aos critérios limitados de versão do recorte (93 claims e 14 limiares), 58 ficam fora, nomeadas em `docs/dados/a33-identidade-derivada/medicao.json`. C1 não fica destravado: a claim de Wijnmalen escolhida tem composição repetida. Não é identidade de claim estável nem atestado de fidelidade à publicação; identificador não certifica o conteúdo. Rastreabilidade separada com endereço, texto e SHA; endereço não é identidade. Todos os 165 `trechoId` permanecem nulos, os 161 pendentes e as 19 divergências de A.16 permanecem. Medido: tsc 0, 18 suítes e 240 testes, 12 controles novos. Nenhuma geração, adoção, alteração da base, reingestão ou avanço de `main`; predição não testada. Avaliação e exclusões registradas em `docs/imprecisoes-parecer-ia.md`. ⚠ **Composição revisada medida em `059d3fc`, protocolo anterior `4f2a42a`, sem adoção:** H-U versiona conteúdo completo; H-T, projeção por tipo. As duas cobrem 164/165 sob os mínimos declarados, contra 107 antes; H-T não cobre mais. A exclusão é o benchmark de `saiyed2023_ceoPowerUET`, com sete campos quantitativos nulos. Os seis grupos se separam; seis pares negativos de cópias mantêm candidata igual sob endereço diferente. No complementar separado, cinco vetores de 36 artigos, 529 unidades, ambas cobrem 529; os 128 IDs de fórmulas são nativos e ficam literais. A claim de Wijnmalen de C1 é discriminada, assim como os outros dois chunks; com o contexto estático, C1 continua em 164/165. Identidade de versão muda com redação, não é identidade estável nem certificado de fidelidade. Todos os trechoId continuam nulos, com 161 pendências de publicação e 19 divergências preservadas. Medido: tsc 0, 19 suítes e 261 testes. Sem geração, alteração da base, reingestão ou avanço de main. Números, controles e limites em `docs/dados/a33-identidade-revisada/` e no registro experimental. ⚠ **H-T APLICADO em `307304d`, protocolo anterior `fe095c4`, decisão do autor:** as **164** unidades elegíveis recebem `trechoId` com a composição **completa** `A33-por-tipo-v2` — `articleId`, `tipo`, `idNativo` e `versaoSha256`, separados por RS — e não apenas o resumo. O ID nativo das 36 fórmulas fica literal; nas 101 claims o componente é `["ausente"]`, sem substituto. Propagado por `enderecoNaBase` a **532** campos nos três artefatos, inclusive os três chunks de C1: 165 unidades alcançadas, **0 órfãos**, **164 valores distintos**. A única exceção é o benchmark de `saiyed2023_ceoPowerUET`, com `trechoId: null` e motivo explícito: insuficiência para o **critério** de benchmark, com os sete campos quantitativos nulos, **e não defeito da base**; ele **permanece no contexto estático dos três casos**. **O arquivo mudou porque ganhou um campo; o texto não mudou, e foi medido:** os resumos estrutural e de texto coincidem com os capturados antes nos três artefatos, com **zero divergência nas 329 raízes** comparadas, e o identificador novo não entra no controle enquanto os antigos continuam nele. O novo identificador **não integra o texto entregue ao modelo**, conferido no contexto montado. Um `trechoId` recalculado de forma **independente** em Python, a partir do snapshot e só das regras do registro, sem a função de aplicação e sem a preimagem armazenada, com valor completo e componentes conferidos. H-U preservado na auditoria, sem virar identificador. `lib/rag/articles/` inalterada no diff. Medido depois de executar: tsc 0, **20 suítes e 275 testes**, com três contraexemplos conferidos. ⚠ **A atribuição de identidade não transforma as 161 pendências em evidências conferidas**, e as 19 divergências de A.16 seguem abertas. **Próximo passo: conferir as evidências contra as publicações, começando pelos trechos de C1.** Geração continua condicionada, predição **não testada**, `main` em `33c1fdf`. ⚠ **Tres correcoes depois da auditoria, em `e1bdc6f`:** (1) o registro passa a **reconhecer o desvio de branch** — a branch corrente **nao era** a designada, a aplicacao **prosseguiu em vez de parar** embora isso fosse condicao de parada, e o historico foi preservado, entao a entrega e **integravel sem reescrita**; chamar aquela conferencia de "bloqueio" descrevia o procedimento pelo que deveria ter feito. (2) **Metadado administrativo que a adocao tornou falso**, reposto separando ausencia de ID **nativo** de identidade **derivada**: `semTrechoId` de 165 para 1, `identidade` vira objeto **por unidade** com `nativaNaBase` e `derivada`, `semIdentificadorRecuperavel` perde as 164 pendencias resolvidas, e `casos.json` e `C1-recuperacao.json` acompanham a adocao. ⚠ **O vetor NAO foi renomeado para "sem ID nativo": as 36 formulas TEM ID nativo**, e a ausencia medida vale para as 101 claims, 18 limiares e 10 benchmarks, com esse alcance. Saiyed **aponta** para `aplicacao.json`, **sem duplicar** a justificativa. O controle estrutural passa a operar por **excecoes ENUMERADAS**, 172 caminhos com valor anterior e posterior fixados por SHA-256 dos dois lados: **341 folhas diferem, 0 nao cobertas, 0 sem uso, 0 com valor errado**, e `contexto-estatico.json` fica com **ZERO** diferencas. Os resumos **textuais** anteriores continuam coincidindo, 0 raizes divergentes em 329. (3) **O teste de C1 verificava tres de NOVE objetos de chunk**: as seis copias nao carregam `trechoId` e se vinculam por `chunk.id`, percurso que ficava sem protecao. Agora cobre as nove, com associacao unica, o H-T correto, **comparacao de conteudo** — ID correto com conteudo trocado **reprova** — e controles de vinculo ausente e ambiguo; o relato foi corrigido. Ocorrencias de `trechoId`: 532 para 368. Medido depois de executar: tsc 0, **20 suites e 283 testes**, com o instrumento novo falhando na estreia e corrigido na conferencia a mao, mais dois contraexemplos sobre os dados reais. ⚠ **Corrigir metadado administrativo nao confere publicacao alguma:** as 161 pendencias e as 19 divergencias de A.16 seguem, e a predicao continua **nao testada**. `main` em `33c1fdf`. ⚠ **O CI reprovou antes de passar, e a causa era minha:** `25fb2c0` ficou vermelho nas duas branches, execucoes `35109761970` e `35109812324`, com **4 testes falhando de 283**. Os quatro do controle estrutural liam a referencia por `git show a84c35b`, e o workflow usa `actions/checkout@v4` **sem `fetch-depth`**, isto e, **clone raso**, onde aquele commit nao existe; reproduzido fora do CI, `git show` sai **128**. ⚠ **A suite passava aqui porque aqui ha historico: "medido depois de executar" era verdade e nao bastava, porque faltou medir no AMBIENTE do CI.** Corrigido em `46ac135`: o controle **reconstroi** o estado anterior desfazendo so os caminhos enumerados e compara com os resumos de referencia gravados, global e por chave; **nenhum teste chama git**, e a enumeracao passa a guardar valor anterior e posterior COMPLETOS. ⚠ **A garantia mudou de lugar e o teste diz isso:** quem pega alteracao fora da lista e a comparacao com a referencia, e nao `naoCobertas`, que nesta forma seria vacuo. Medido tambem na condicao do CI, em **clone raso de um commit**: tsc 0, 20 suites e 283 testes, saida 0. CI `35114443428` e `35114607612` observados **completed/success** em `46ac1359134fe3a91000c25b9f15c0369e5f63d0`, na branch de trabalho e na de integracao. **`integra/a30-registros` avancou por FAST-FORWARD ate esse SHA**, local e remoto, sem `--force`, rebase ou `amend`, com `git log --merges` vazio e os **seis** commits preservados. `main` permanece em `33c1fdf`. ⚠ **Conferencia dos tres trechos de C1 contra as publicacoes, em `37ae5c3`: o resultado e SEM ACESSO, e nenhuma unidade mudou de estado.** **4 → 4 conferidas** e **161 → 161 pendentes**, sobre 165 unidades DISTINTAS sem somar copias; **o total NAO passou de quatro para sete**. Associacao unica nos tres, por `articleId`, `trechoId` e `enderecoNaBase`, nos indices 47, 86 e 100, com os nove campos registrados. **Nenhum dos tres esta entre as 19 divergencias de A.16**, e o caso mede por que a marca e por TRECHO e nao por obra: `wijnmalen2007_bocr` tem divergencias em `key_claims[2]`, `[3]` e `[4]`, e o trecho de C1 e `key_claims[0]`. **As quatro vias se esgotaram:** nenhum PDF local, e **dezesseis hosts recusados** pelo proxy de egresso por politica de organizacao, entre eles ScienceDirect, doi.org, ResearchGate, arXiv, JSTOR e Crossref. Dois quase-acertos foram recusados de proposito por serem **outra obra**: o comentario de Wedley sobre Saaty 1977 e duas outras publicacoes de Wijnmalen. Resumo de buscador nao foi usado como conteudo. **Resultado por campo, separado da sustentacao:** `verbatim_quote` e `evidence.quote` **nao conferidos** nos tres, e sustentacao da `claim` **inconclusiva** nos tres. ⚠ **Busca sem achado e "nao conferido", nunca "nao confere"**, e o alcance da busca esta registrado com o que ficou de fora. **Achado:** o registro anterior de `forman1998_aggregating` `key_claims[2]` **nao traz veredito separado de sustentacao**, porque separar correspondencia de recorte de sustentacao e regra POSTERIOR a ele; a unidade segue conferida e supri-la exige a publicacao. Medido depois de executar: tsc 0, **21 suites e 290 testes**, e o mesmo em clone raso de um commit. CI `35117934621` e `35117937482` observados **completed/success**; `main` em `33c1fdf`. **Proximo passo, e e decisao do autor: as duas publicacoes pendentes exigem via que este ambiente nao tem** — credencial institucional, copia do autor, ou maquina sem a politica de egresso. ⚠ **Rodada de fechamento pedida com os PDFs em maos NAO conferiu as publicacoes, e a razao e de ambiente**, em `a3d0f88`: a sessao **nao e a maquina Windows**. Medido: Ubuntu 24.04.4 em conteiner, **nenhum** `C:\AHP-BOCR\preservado`, `/mnt/c` ou montagem de disco Windows; `find` por `*.pdf` em todo o sistema devolve **dois** arquivos de infraestrutura; `/mnt/attach` e `/mnt/user-data/working` vazios. ⚠ **Falso positivo descartado:** caminhos com `saaty1977` e `wijnmalen2007` em `/tmp/jest_0` sao **cache de transpilacao do jest** sobre os `.ts` do repositorio, nao publicacoes. Nenhum resultado foi inventado. **O que FOI executado sao os dois ajustes do registro:** (1) ausencia de avaliacao deixa de ser resultado negativo — `doisCamposConferem`, `claimSustentada` e `semLocalizadorDivergente` passam a **nao avaliado** com motivo, e **`semDivergenciaA16: true` permanece** porque FOI conferido; (2) **tres indicadores distintos**: contagem historica preservada com 4 conferidas e 161 pendentes, Forman entre as quatro e **3** anteriores fora desta revisao; **0** conferencias novas na rodada anterior; e **0 de 3** trechos de C1 com atendimento demonstrado ao criterio atual. ⚠ **As tres anteriores fora da revisao NAO passam automaticamente a atender aos criterios novos.** Os sete testes da rodada anterior seguem como historico datado, e dois novos cobrem os ajustes sem substituir aquela evidencia, conferidos por tres contraexemplos. Medido depois de executar: tsc 0, **21 suites e 292 testes**, e o mesmo em clone raso de um commit. CI `35120552613` e `35120556094` observados **completed/success** em `a3d0f88eda326ce2240565c52103c11019b6725b`; `main` em `33c1fdf`. **Destrava executando a partir da maquina que tem os PDFs, ou entregando os tres arquivos a uma sessao que os alcance.** ⚠ **Comparacao RECEBIDA sobre os tres trechos de C1, gravada em `835cd55`: esta execucao NAO consultou as publicacoes**, e a comparacao foi feita **sobre TEXTO EXTRAIDO** por outra sessao. Proveniencia declarada **POR AFIRMACAO**, em quatro entradas; ⚠ **A RASTREABILIDADE DOS PDFs NAO ESTA DEMONSTRADA:** SHA-256, versao e contagem de paginas estao **recebidos e ainda NAO verificados**, sem localizador da evidencia original — entraram pelo texto do prompt, e a mensagem ou artefato de origem nao e identificavel aqui. **Nao esta demonstrado** que cada SHA-256 corresponda ao arquivo lido, que a pagina impressa ocupe a posicao declarada naquele arquivo, nem que a extracao tenha vindo dele: **o vinculo extracao-PDF nao esta estabelecido**. **DOI e PII identificam o ARTIGO, o ISSN identifica o PERIODICO, e nenhum deles identifica o arquivo**, e por isso `versaoDoArquivoConsultado` e **null**. Pagina impressa e posicao no PDF em campos **distintos**, com diferenca declarada de 233, 891 e 164; ⚠ **conferir essa subtracao e aritmetica, e nao demonstra a posicao no arquivo**. **Forman `key_claims[2]`: confere nos dois campos, claim sustentada**, sobre extracao, com a conferencia historica preservada no seu alcance e a inspecao visual da pagina 167 ainda por fazer. **Wijnmalen `key_claims[0]`: NAO CONFERE nos dois campos**, por **omissao de `however` sem sinalizar a supressao**; **claim SUSTENTADA**; unidade **pendente por nao correspondencia textual**; o achado **segue para A.16** e **a base NAO e corrigida aqui**. ⚠ **Ele NAO se soma numericamente as 19**: aquelas registram divergencia ENTRE CAMPOS DA BASE, e aqui os dois campos **coincidem entre si** e diferem do texto extraido. **A contagem historica das 19 permanece inalterada.** **Saaty `key_claims[0]`: pendente de inspecao visual**, com **um item so** — a expressao matematica e a capitalizacao do inicio — e **nada gravado como conclusao**. Medido NESTA execucao, do lado da base: o recorte de Wijnmalen nao traz `however`; o de Forman sinaliza a elisao por reticencias; e o de Saaty carrega **notacao LaTeX** `\lambda_{max} = n`, razao tecnica da pendencia, porque o recorte ja e transcricao e so a pagina impressa separa transcricao fiel de divergencia. ⚠ **Contagens inalteradas e historicas: 4 e 161 sobre 165 distintas, ZERO conferencias novas declaradas, nenhum total antecipado e nenhuma subtracao**; o indicador de atendimento ao criterio atual continua **0 de 3**. O registro datado da rodada sem acesso **nao foi substituido**. Medido depois de executar: tsc 0, **21 suites e 297 testes**, e o mesmo em clone raso de um commit, com quatro contraexemplos. CI `35124511720` e `35124514932` observados **completed/success** em `835cd55c0196d90d2d365bc7845d2773385fef50`. ⚠ **A conferencia de C1 NAO esta concluida.** `main` em `33c1fdf`. **Rodada de MEDICAO na maquina Windows, em 16/09/2026, `a00fb5e` e `ae580c6`, branch `inspecao/saaty-237`:** os tres PDFs foram localizados em `C:\AHP-BOCR\preservado\`, **fora da arvore**, com caminho e tamanho; os **tres SHA-256 foram recalculados aqui e coincidem** com os recebidos, e o registro separa os dois fatos: o recalculo verifica o arquivo acessado, a coincidencia confirma a igualdade e **nao demonstra o vinculo com a extracao anterior**, que continua **nao estabelecido**. **As tres posicoes foram VERIFICADAS** pelo numero impresso no cabecalho, com as vizinhas: 237 na 4, 899 na 8, 167 na 3, coincidentes. **Saaty p. 237 inspecionado visualmente**, com PDFium a 300 dpi, depois de confirmar que todo o texto OCR da pagina e invisivel: o impresso traz *a* **minusculo** depois de *It turns out that*, e lambda com subindice max; a camada OCR deste arquivo le **`h max = a`**; o recorte e identico a base. **Quatro transformacoes na expressao**, tres na extracao e a conversao para LaTeX na base, com os mesmos simbolos; e **origem nao determinada** para as que estao na base, porque a base nao registra o percurso do texto ate ela, com a pendencia nomeada. **`verbatim_quote` e `evidence.quote`: NAO CONFERE**, por alteracao de caixa nao sinalizada na primeira letra; **claim SUSTENTADA**; localizador compativel; **unidade continua pendente**, achado encaminhado a A.16 **fora das 19**, base **nao** corrigida. A notacao LaTeX e aceita como **representacao equivalente escolhida**, que preserva os simbolos e a relacao matematica, com a transformacao declarada; o *nao confere* vem da caixa, e **nao ha decisao pendente** sobre ele (justificativa corrigida depois). **ISSN passa a identificador do PERIODICO**, e o Saaty fica com **"identificador de artigo nao registrado"**; o titulo nos metadados do arquivo traz uma cadeia com forma de PII, registrada **sem adocao**. Contagens inalteradas, **4 e 161**, e o indicador de criterio atual segue **0 de 3**. Medido: `tsc` 0; nesta maquina **305 testes, 302 passando**, com tres falhas **anteriores** a rodada -- a do CRLF, **ambiental com causa medida** (`core.autocrlf=true`), e dois timeouts intermitentes, **comportamento observado SEM causa estabelecida** (corrigido na rodada de Wijnmalen e Forman, que antes dizia ambientais para as tres) --; em **clone raso LF de um commit, 305 de 305**. CI `35135930664` **completed/success** em `ae580c641a821800480c719e32cda45f183d2769`. ⚠ **A conferencia de C1 continua NAO concluida:** Wijnmalen segue com nao correspondencia **na extracao**, e os textos de Wijnmalen e Forman **nao** foram inspecionados visualmente. **Rodada de INSPECAO de Wijnmalen p. 899 e Forman p. 167, em 16/09/2026, `3fc4581` e `781fb4f`, branch `inspecao/wijnmalen-forman`:** hashes recalculados e coincidentes; **nova extracao vinculada a eles**, por tres vias, com proveniencia propria, e a extracao antiga com o seu limite historico. Procedimento por tipo: Wijnmalen e **texto vetorial visivel**, sem exclusao de camada; Forman e **digitalizacao**, com a separacao verificada. **Paragrafo completo** lido nos dois. **Wijnmalen: NAO CONFERE** nos dois campos -- omissao de *however*, caixa `S` -> `s` e truncamento no meio da frase, todos nao sinalizados e de **origem nao determinada** --; **claim PARCIALMENTE SUSTENTADA**, porque a claim exige comensurabilidade das prioridades dos fatores **antes de qualquer sintese**, enquanto o artigo admite **quociente significativo quando os produtos tem unidades compativeis**, sem exigir que os quatro totais sejam individualmente iguais; unidade **pendente**, achado para A.16 **fora das 19**; o mesmo texto e tambem o `evidence.quote` de `key_claims[3]`, uma das 19, registrado como alcance e **nao** conferido; a repeticao demonstra **repeticao da divergencia**, e nao a origem. **Forman: CONFERE** nos dois campos, com tres transformacoes declaradas -- aspas tipograficas para ASCII, **enfase italica de *must* nao reproduzida** e hifenizacao desfeita --; **claim SUSTENTADA** pela conclusao explicita do autor para AIJ, com as premissas, reciprocidade e Aczel e Saaty sob Pareto e homogeneidade, registradas e **nao** supridas. Contagem historica **inalterada, 4 e 161**; **indicador de criterio atual de 0 para 1 de 3**, pela medicao; a perda do italico em Forman e **aceita neste recorte**, porque *must* permanece. ⚠ **C1 nao se anuncia liberado:** as tres inspecoes estao feitas, dois trechos seguem pendentes, e **nao ha decisao de criterio pendente**. **Pendencia tecnica em separado:** os dois testes intermitentes de `rag-diagnostico-regressao`, **comportamento observado sem causa estabelecida**, nada corrigido. Medido: `tsc` 0; nesta maquina **312 testes, 309 passando**, com as tres falhas ja registradas; em **clone raso LF de um commit, 312 de 312**. CI `35141405974` **completed/success** em `781fb4fb303cd67e7eaa2514d40ac776d9e69c18`; `3fc4581` sem execucao propria. `main` em `33c1fdf`. **Correcao de QUATRO justificativas, em 16/09/2026, `4309f7e`, sem mudar nenhum resultado**, conferido campo a campo: Wijnmalen segue nao confere e parcialmente sustentada, com a ressalva na redacao prescrita, a Tabela 6 separando ordenacao de rentabilidade, os produtos iguais sem dispensa de comensurabilidade, e *guarantee* sem excluir caso particular; Forman segue confere, porque *must* permanece, sem recorrer ao *strictly*, como decisao sobre este caso e nao regra geral; Saaty segue nao confere pela caixa, com a notacao LaTeX como **representacao equivalente escolhida**; e a repeticao em `key_claims[3]` demonstra **repeticao da divergencia, nao a origem**, que segue nao determinada, sem atribuicao a v3.0. Contagem por familia, antes e depois: impossibilidade do meio 10 -> 1, atribuicao a v3.0 21 -> 0, *strictly* 3 -> 0, enfase pendente 14 -> 2, decisao pendente sobre Saaty 4 -> 0, atenuantes 14 -> 5; o que resta sao proibicoes no teste e copias literais de mensagens de commit. 312 testes, 309 aqui e 312 no clone raso LF. **1 de 3**, **4 e 161**. CI `35153121514` **completed/success** em `4309f7e662091f050c220c458d042d856e342352`. C1 nao se anuncia liberado. `main` em `33c1fdf`. **Correção na base dos dois trechos que não conferem, por A.16, em 16/09/2026:** `8b057d9` e `b567d99`, predição em `3db365c`. O contexto estático das seções de consistência e BOCR passa a trazer o texto impresso e a claim nova, e **deixa de coincidir com `contexto-estatico.json`**; C1, cujos chunks fixados trazem o texto antigo, fica **fora** da predição. **A versão nova do snapshot é etapa própria, antes da etapa 4**, e **não torna os trechos conferidos**: primeiro se auditam as correções da base; depois se propagam os conteúdos aprovados, se recalculam as identidades afetadas e se verificam as cópias. O snapshot anterior conserva os seus resultados históricos — **4 e 161**, e **1 de 3**. ⚠ **As quatro credenciais não são requisito da etapa 4:** C1 e C3 usam recuperação simulada e C2 a tem desligada; a geração exige acesso ao modelo real, Voyage e Upstash não são chamados no ensaio, e a presença de variáveis no `.env.local` desta máquina não demonstra autenticação. Publicada em 17/09/2026, um commit por push, com CI verde em cada um, e **integrada em `integra/a30-registros` até `4cb14f5`**, por fast-forward, local e remoto, CI `35217659825` verde. ⚠ **Pendência antes de qualquer geração: delimitar P1**, porque omitir *however* numa paráfrase não é erro por si só, e o critério tem de separar reprodução apresentada como citação literal de reformulação fiel. **Próxima tarefa, não iniciada:** a nova versão do snapshot. A saída do modelo continua não observada. `main` em `33c1fdf`. **Nova versão do snapshot, em 17/09/2026, `c87e337`, branch `snapshot/a33-etapa4-v2`:** `8f94341` incorporado em `integra/a30-registros` por fast-forward, sem commit de narração. A v1 em `docs/dados/a33-etapa4/` fica **integralmente inalterada**; a v2, em `docs/dados/a33-etapa4-v2/`, aponta para ela e declara a base corrigida, `8f94341`. **Derivadas da base, e não presumidas:** as seis alterações aprovadas, nas unidades 47, 86 e 89; contra `344d631`, zero. **56 exceções enumeradas**, cada uma ligada a alteração aprovada ou a consequência nomeada; **311 de 329 raízes de texto idênticas**. H-T **recalculado nas três**, por campo da composição; as 162 mantêm o `trechoId`; 368 ocorrências e seis cópias por `chunk.id`, sem órfão. Montagem real com clientes simulados só para captura, `fetch` com zero chamadas, sem parecer. **Nenhuma transição**: 4 e 161; **C1 na v2 em 1 de 3**, calculado dos registros, porque o conteúdo novo de Saaty e Wijnmalen `[0]` não tem conclusão registrada; Wijnmalen `[3]` **parcialmente pendente**. `tsc` 0; **23 suítes e 361 testes**, 358 aqui e 361 no clone raso LF; CI `35231221204` verde. **Pendências antes de gerar:** delimitar P1, inclusive para C1; conclusões sobre o conteúdo novo. `main` em `33c1fdf`. **Três ajustes na v2, em 17/09/2026, `82057ec`, `2087b73` e `14d2ba5`:** (1) a montagem passou a usar o **payload de referência**, `docs/calculations-13jul2026.json`, transformado em requisição por `scripts/a33-payload-referencia.cjs`, que espelha a tela e declara 31 origens, conferido por 23 âncoras; ⚠ **achado, não resolvido:** sem análise de qualidade, a tela declara os 12 respondentes **confiáveis** e a pontuação automática sai **100/100**, terreno de A.12, pendente antes de gerar; (2) `verificacaoCodigo` corrigido **só na v2**, com o alcance medido, e o manifesto declara que o `sha` do endereço registra a base de onde o conteúdo foi lido; (3) **encadeamento por novo `trechoId`** — inspeção, correção aprovada em A.16 e propagação —, sem reabrir a inspeção: Saaty e Wijnmalen `[0]` **conferidos** na v2, Wijnmalen `[3]` **parcialmente pendente**; **v2 com 6 e 159, e C1 em 3 de 3; a v1 fica histórica, com 4 e 161, e 1 de 3**. ⚠ **Supera, para a v2, a frase anterior sobre conclusões ausentes.** `tsc` 0; **377 testes**, 374 aqui e 377 no clone raso LF; CIs verdes. **Pendências antes de gerar:** P1, a qualidade da requisição de referência e A.16 em `[3]`. `main` em `33c1fdf`. **Requisição de referência r2, em 17/09/2026, `76b93e8`:** `metadata.projectName` → `projectName`, por mapeamento declarado — ⚠ **o nome está no arquivo**, e a r1 errava ao dizer que não —; inventário de 30 campos pela **disponibilidade da informação**, separada do valor enviado: 8 disponíveis, 5 derivados, **16 indisponíveis**, cujo valor enviado não é informação medida, e **1 comprovadamente vazio**, `exclusionInfo`, com a evidência `metadata.excludedRespondentIds = []`, que é a fonte da tela; `REVISAR: 0` à parte, como constante do código. **Fallback de qualidade preservado**, e o efeito dele, 12 confiáveis e 100/100, é **pendência de A.12**. Montagem dos três casos por captura e interrupção; as âncoras medem fragmentos, **não** equivalência. r1 preservada em artefato identificado; r2 com resumo `6541ecd4…`, comum aos três casos. `tsc` 0; **381 testes**, 380 aqui e 381 no clone raso LF; CI verde. `main` em `33c1fdf`. |
| A.34 | **Gestão e versionamento das fontes do RAG.** Fluxo de **inclusão, atualização e retirada** de publicação: cadastro, conferência dos **dados bibliográficos** e dos **direitos de uso**, preparo dos trechos, revisão de fidelidade, e **só então** disponibilização para recuperação. ⚠ **Requisito de XAI, com as etapas distinguidas:** o parecer permite identificar as evidências **recuperadas**, as **fornecidas ao modelo** e as **citadas no parecer**, que são coisas diferentes, e **registrar que algo entrou no contexto não demonstra que sustentou a resposta**. A proposta **define esses vínculos** e **preserva a versão correspondente aos pareceres anteriores**, de modo que **incluir artigo novo não altere retrospectivamente a rastreabilidade**. Para isso, **auditar o contrato atual e definir os registros necessários**. **Antecedente aproveitável:** H-T foi aplicado aos dados preparados de A.33 e **identifica uma versão da projeção de conteúdo por tipo**; alterar um campo incluído nessa composição **muda a identidade**, e classificação e rótulos **excluídos não a alteram**. A proposta **avalia o seu aproveitamento no fluxo permanente**. ⚠ **O índice é cópia que desatualiza:** o fluxo diz **quando a reingestão ocorre** e **como se sabe que o índice corresponde à base**, sob A.18. ⚠ **Direitos de uso distinguem três permissões**, a apurar **por fonte**: o que pode ser **armazenado**, o que pode ser **entregue ao modelo** e o que pode **aparecer na tela**. ⚠ **Indisponibilidade do índice é requisito operacional:** o fluxo **distingue aprovação editorial de disponibilização na recuperação** e **registra sincronização pendente** quando o índice estiver indisponível, de modo que **cadastrar uma fonte não equivale a declarar que ela já está recuperável**. **Avaliar o efeito de novas fontes exige definir o protocolo e registrar a linha de base ANTES da inclusão**, **identificando quais medições existentes podem ser aproveitadas**: mais artigos não significam pareceres melhores. **NÃO DUPLICA A.16 nem A.18:** a revisão da base existente e a reingestão são delas. **Acréscimo de 21/09/2026, orientação do autor:** ⚠ **`refsUsed` conta referências carregadas, não usadas:** `route.ts:1527` atribui `getKnowledgeStats().totalRefs`, e a tela, em `components/ParecerAISection.tsx`, linhas 347 e 381, exibe `refsUsed \|\| 24`, que **substitui ausência e também zero**, porque `0 \|\| 24` resulta em 24; é consequência da expressão, e **não demonstra que um zero tenha ocorrido em produção**; lido em `ebfc9dd` na sessão de análise, **a reconfirmar por leitura**. A arquitetura distingue **referências disponíveis**, **selecionadas e efetivamente inseridas no contexto**, **citadas na saída**, e **afirmações cuja sustentação foi conferida**. **Por execução, um manifesto vincula requisição, contexto, evidências, versões, configuração e saída**; isso permite reconstruir o ensaio, e **não garante reproduzir a redação do modelo**. **Entrega:** proposta de gestão e versionamento, **que vai à revisão antes da implementação** |
...
**Dependência técnica**, que não depende de escolha. Nada aqui é novo: cada linha
consolida o que já está registrado na tarefa correspondente.

| Item | Depende de | Razão |
|---|---|---|
| A.18 | A.30 | com a recuperação falhando, o teste que A.18 prevê daria vazio por outra razão |
| A.18 | A.16 e A.26 | conteúdo e ingestor antes de pagar a reingestão, para pagar uma vez |
| A.2, commit (4) | A.29 | `lib/knowledge.ts` é o legado que A.29 remove |
| A.33 | A.16, **só para a verificação** | ⚠ **Justificativa TROCADA em 15/09/2026.** A antiga, conteúdo do campo citado, **caiu com o campo de página**: nada do que A.33 injeta depende de qual dos dois campos está correto, então **a implementação NÃO depende de A.16**. A dependência efetiva é outra e se confirma: o `verbatim_quote` é a **referência da conferência de sustentação**, porque transcrição sem sinalização só se detecta comparando a afirmação com o trecho guardado. Com a divergência aberta, a linha sai **inconclusiva**, sem escolher lado. **Medido nesta rodada: 26 divergências entre 138 claims com os dois campos, contra as 13 entre 123 do registro vigente; não reconciliado, e reconciliar é tarefa de A.16.** **Não depende de A.18:** a injeção estática não passa pelo índice semântico |

**Prioridade escolhida, 21/09/2026**, que é decisão do autor e muda sem que nada
no código mude. ⚠ **Estado sujeito a revisão.** Substitui a prioridade de
13/09/2026, que era A.30, A.31 e A.33.

1. ✅ **Medição da cadeia de fallback de `rule`**, em A.15 e A.16, integrada em
   `e14df56`. **O fallback é latente:** nenhuma das 138 unidades da base viva o
   aciona, e as 138 fornecem `rule` pelo `evidence.quote`. A correção posterior
   deve **preservar o contexto montado atual**, e tratar registros futuros
   incompletos, só com espaços ou sem o objeto `evidence`.
2. **Bloqueios de A.12.**
3. **Correções delimitadas**, com predição registrada antes quando alterarem o que
   o modelo recebe.
4. **Conferência de P1** contra a requisição vigente, incluindo a aplicabilidade a
   C1.
5. **Só então, a preparação da autorização da geração de A.33.**

A proposta de arquitetura de manutenção e rastreabilidade entra em **A.34**,
aproveitando esses resultados.

⚠ **A.31 permanece aberta.** A retirada dela desta lista **não significa conclusão
nem aprovação da seleção temática vigente**. **Antes de autorizar a geração de
A.33, registre como essa pendência afeta o alcance do ensaio e das conclusões que
ele permite.**

⚠ **A ordem é prioridade, não dependência técnica**, salvo onde a tabela acima a
registra.

**Pendências que condicionam a sequência, 21/09/2026.** ⚠ **Estado sujeito a
revisão.**

- **A.12, bloqueios:** definir **qual fonte estabelece o universo de respondentes**
  a avaliar, **incluindo exclusões legítimas**; verificar **cobertura e coerência**
  entre avaliações individuais e agregados; **manter nota global e veredicto
  suspensos** quando a avaliação necessária estiver **ausente, incompleta ou
  apresentar contradição não resolvida que comprometa a classificação**, **com o
  motivo registrado**, e **preservar os cálculos AHP-BOCR cuja validade não seja
  afetada**; testar **a resposta do handler e a apresentação
  efetiva ao gestor**, além das funções auxiliares. ⚠ **A regra da ausência no
  `CLAUDE.md` não demonstra a implementação.**
- **Correção de `rule`, depois da medição:** preservar **campo de origem do texto,
  natureza do conteúdo, identidade e versão da unidade, e estado da conferência
  bibliográfica**. **Afirmação do preparador não adquire autoridade de publicação
  por preencher campo vazio.** ⚠ **Isso não autoriza considerar todo
  `verbatim_quote` infiel, nem retirar os originais do contexto.**
- **Autoridade da nota extraída do parecer:** a rota permite que ela substitua a
  classificação automática. **Revisão própria**, sem confundir essa nota com os
  resultados numéricos do AHP-BOCR.
- **Wijnmalen `key_claims[3]`, em A.16:** `verbatim_quote` e claim seguem abertos.
- **A.16, alcance da lista de 19:** ela cobre o **escopo preparado de A.33**, 101
  `key_claims`. Na base viva, pelo mesmo critério, são **26**; as sete a mais
  estão fora daquele escopo. **Instrumento produtor da lista histórica não
  localizado no alcance consultado**, e a reprodução atual de 101 de 101 demonstra
  consistência com as cópias, não a procedência do cálculo histórico. Registro em
  `docs/imprecisoes-parecer-ia.md`, integrado em `e14df56`.

**Orientação arquitetural vigente, 21/09/2026.** ⚠ **Orientação atual, sujeita a
revisão, e não regra permanente.** A regra permanente, separação de
responsabilidades e mudança estrutural não apresentada como neutra sem verificação,
está no `CLAUDE.md`.

A rota do parecer reúne hoje normalização, classificação, recuperação, montagem,
chamada ao modelo, validação e resposta. Fronteiras internas recomendadas, no mesmo
sistema:

| Responsabilidade | Contrato esperado |
|---|---|
| Entrada | dados, origem, disponibilidade e inconsistências explícitas |
| Cálculo e classificação | resultados determinísticos separados da interpretação do LLM |
| Montagem | contexto e manifesto das evidências fornecidas, verificáveis sem geração |
| Geração | recebe contexto fechado e devolve saída identificada |
| Validação e apresentação | expõem o que foi verificado, os limites e as pendências |

⚠ **Não abrir refatoração geral.** Extrair essas responsabilidades **gradualmente,
nas correções necessárias**, demonstrando preservação de comportamento quando essa
for a intenção. **Não reescrever o sistema nem trocar de modelo agora.**

**A.28 corre como frente independente, e não se condiciona à conclusão do RAG.** O
próximo passo dela é decidir a Fase 2 a partir do diagnóstico registrado, com a
especificação em `docs/validacao-externa-especificacao.md`. As cinco opções da
seção 3 daquele documento, inclusive a remoção da "Citação para dissertação",
**seguem como recomendação até a Fase 2 formalizá-las**.
