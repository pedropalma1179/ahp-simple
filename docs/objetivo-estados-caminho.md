...
Documento âncora. Revisado em 11/09/2026 sobre o commit `c8ba368`.
...
| A.13 | **Corrigir o localizador `p. 271` no código.** O primeiro commit de A.10 corrigiu a página no `system-prompt.ts`, mas **a correção ficou pela metade**: um grep em 11/09/2026 encontrou quatro ocorrências no código, todas com a mesma página errada. A correta é **248**. Onde: `components/BOCRConsistencyMatrix.tsx` (comentário do `CR_THRESHOLD`); `app/api/ai-reviewer/bias-detection.ts`, duas vezes, uma delas afirmando `Saaty (1977, p. 271): "A consistency ratio of 0.10 or less is acceptable"`; e `lib/rag/citation-whitelist.ts`, como exemplo de casamento. ⚠ O do `citation-whitelist` é o mais delicado: é exemplo de documentação de um verificador, e trocá-lo pode invalidar o exemplo. Conferir antes. Achado durante A.5, registrado sem corrigir de passagem |
